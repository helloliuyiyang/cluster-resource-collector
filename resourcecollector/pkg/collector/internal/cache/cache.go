...
import (
	...
	"k8s.io/kubernetes/resourcecollector/pkg/collector/utils/sets"
	"k8s.io/kubernetes/resourcecollector/pkg/collector/utils/wait"
)
...
func (cache *collectorCache) moveNodeInfoToHead(nodeID string) {
...
// linked list.
// We assume cache lock is already acquired.
func (cache *collectorCache) removeNodeInfoFromList(nodeID string) {
	ni, ok := cache.nodes[nodeID]
	if !ok {
		logger.Errorf("No NodeInfo with name %v found in the cache", nodeID)
		return
...
func (cache *collectorCache) UpdateSnapshot(nodeSnapshot *Snapshot) error {
	cache.mu.Lock()
...
func (cache *collectorCache) updateNodeInfoSnapshotList(snapshot *Snapshot, updateAll bool) {
	if updateAll {
...
func (cache *collectorCache) removeDeletedNodesFromSnapshot(snapshot *Snapshot) {
...
func (cache *collectorCache) AddNode(node *types.SiteNode) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *collectorCache) UpdateNode(oldNode, newNode *types.SiteNode) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *collectorCache) RemoveNode(node *types.SiteNode) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *collectorCache) updateRegionToNode(newNode *types.SiteNode) {
...
func (cache *collectorCache) deleteRegionToNode(newNode *types.SiteNode) {
	_, ok := cache.regionToNode[newNode.Region]
	if !ok {
		return
	}
...
func (cache *collectorCache) UpdateNodeWithEipPool(nodeID string, eipPool *typed.EipPool) error {
...
func (cache *collectorCache) UpdateNodeWithVolumePool(nodeID string, volumePool *typed.RegionVolumePool) error {
	node, ok := cache.nodes[nodeID]
	if !ok {
		return fmt.Errorf("node %v is not found", nodeID)
	}
...
func (cache *collectorCache) UpdateNodeWithResInfo(siteID string, resInfo types.AllResInfo) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *collectorCache) UpdateNodeWithRatio(region string, az string, ratios []types.AllocationRatio) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *collectorCache) PrintString() {
	...
	i := 1
	for _, node := range cache.nodes {
		bytes, err := json.Marshal(node.info)
		if err != nil {
			logger.Warnf("Marshal nodeInfo[%s] err: %s\n", node.info.Node().SiteID, err.Error())
			continue
		}
		str := fmt.Sprintf("collectorCache(%d/%d) siteID[%s], info: %s\n", i, total, node.info.Node().SiteID,
			string(bytes))
		logger.Info(str)
		i++
	}
