...
func (n *NodeInfo) ToString() string {
	ret := fmt.Sprintf("nodeInfo: %#v, totalResources: %#v, RequestedResources: %#v, allocatableFlavor: %#v, "+
		...
		n.AZ.ToString(), n.TotalResources, n.RequestedResources, n.AllocatableFlavor, n.AllocatableSpotFlavor, n.TotalStorage, n.RequestedStorage)
	if n.EipPool != nil {
...
func (n *NodeInfo) AddStack(stack *types.Stack) {
	n.StackSlice = append(n.StackSlice, stack)
...
func (n *NodeInfo) UpdateQos(netMetricData *types.NetMetricDatas) error {
	...
	for _, metric := range netMetricData.MetricDatas {
		qos := metric.Delay + config.QosWeight*metric.Lossrate
...
// Stacks return all StackSlice scheduled (including assumed to be) on this node.
